                ...
                for n in range(largura_kernel):
                    if i + m - altura_padding >= 0 and i + m - altura_padding < altura:
                        if j + n - largura_padding >= 0 and j + n - largura_padding < largura:
                            if kernel[m, n] == 1 and imagem[i + m - altura_padding, j + n - largura_padding] < valor_min:
                                valor_min = imagem[i + m - altura_padding, j + n - largura_padding]
            imagem_erodida[i, j] = valor_min
...
def encontrar_linhas(imagem):
    # Definir o kernel de erosão
    kernel_erosao = np.ones((5, 5), np.uint8)
    
    # Aplicar a erosão manualmente
    imagem_erodida = erosao(imagem, kernel_erosao)
...
def encontrar_palavras(imagem):
    ...
    imagem_dilatada = dilatacao(imagem, kernel_dilatacao)
